...
## ۱۲. اعلان‌ها (Push) در اپ اندروید

اپ اندروید از **Web Push** + **اعلان native اندروید** پشتیبانی می‌کند:

| نوع | چگونه کار می‌کند |
|-----|------------------|
| اعلان‌های سامانه | سرور → Web Push → نوتیفیکیشن سیستم |
| پیام پیام‌رسان | سرور هنگام ارسال پیام → Web Push برای گیرنده |
| اپ باز در پس‌زمین | polling + bridge native (`SchoolManagerNative`) |

### فعال‌سازی برای کاربر

1. اولین بار اپ را باز کند
2. **Allow** روی «اجازه اعلان» (Android 13+) بزند
3. در پاپ‌آپ داخل اپ هم **فعال کردن اعلان** را بزند
4. در پیام‌رسان: تنظیمات → **اعلان مرورگر** روشن باشد

### لوگوی اپ (بدون پس‌زمینه آبی)

هنگام build، PNG لوگو **بدون لایه adaptive آبی** اعمال می‌شود — فقط خود تصویر شفاف روی آیکون.

> بعد از تغییر لوگو حتماً APK را **دوباره build** کنید.
